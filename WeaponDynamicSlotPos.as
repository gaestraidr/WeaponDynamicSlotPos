...
HookReturnCode WDSP_CanCollect( CBaseEntity@ pPickup, CBaseEntity@ pOther, bool& out bResult )
{
    ...
    if (pPlayer is null || pPlayer.edict() is null 
		|| HUDMODE(int(g_PlayerHUDSettings[PlayerID(pPlayer)])) == HUD_ABC_ENCHANCE)
        return HOOK_CONTINUE;
	...
	PlayerLoadout@ loadout = GetLoadout(pPlayer);
	if (loadout is null || loadout.WasInCachePickup(szName))
		return HOOK_CONTINUE;
...
HookReturnCode WDSP_PlayerPostThink( CBasePlayer@ pPlayer )
{
    ...
    if (loadout !is null) {
		...
		if (loadout.m_ReadyState && loadout.m_HasRefreshed) {
			...
			if (loadout.m_SnapshotCalledCount != 0) {
				if (loadout.m_SnapshotCalledCount >= 3) {
